...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        ...
        <flux:button href="{{ route('employees.create') }}" variant="primary" icon="plus">Add Employee</flux:button>
    </div>

    @if (session('status'))
    <flux:text color="green" class="mb-4">{{ session('status') }}</flux:text>
    @endif
            ...
            <flux:table.rows>
                ...
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center py-4">
                        <flux:text color="zinc">No employees found.</flux:text>
                    </flux:table.cell>
                </flux:table.row>
                @endforelse
            </flux:table.rows>
